...
if ( ! class_exists( 'c2c_AddAdminJavaScript_Plugin_040' ) ) :

abstract class c2c_AddAdminJavaScript_Plugin_040 {
	...
	protected function __construct( $version, $id_base, $author_prefix, $file, $plugin_options = array() ) {
		...
		if ( ! file_exists( $file ) )
			die( sprintf( __( 'Invalid file specified for C2C_Plugin: %s', 'add-admin-javascript' ), $file ) );
	 ...
	 * @since 036
	 */
	public function __clone() { _doing_it_wrong( __FUNCTION__, __( 'Cheatin&#8217; huh?', 'add-admin-javascript' ), '036' ); }
	 ...
	 * @since 036
	 */
	public function __wakeup() { _doing_it_wrong( __FUNCTION__, __( 'Cheatin&#8217; huh?', 'add-admin-javascript' ), '036' ); }
	...
	public function options_page_description( $localized_heading_text = '' ) {
		...
		if ( ! $this->disable_contextual_help ) {
			echo '<p class="see-help">' . __( 'See the "Help" link to the top-right of the page for more help.', 'add-admin-javascript' ) . "</p>\n";
		}
	}
	...
	public function sanitize_inputs( $inputs ) {
		...
		if ( isset( $_POST['Reset'] ) ) {
			...
			add_settings_error( 'general', 'settings_reset', __( 'Settings reset.', 'add-admin-javascript' ), 'updated' );
		} else {
			...
			foreach ( $option_names as $opt ) {
				if ( !isset( $inputs[ $opt ] ) ) {
					if ( $this->config[ $opt ]['input'] == 'checkbox' ) {
						...
						$msg = sprintf( __( 'A value is required for: "%s"', 'add-admin-javascript' ), $this->config[ $opt ]['label'] );
	...
	protected function verify_config() {
		...
		foreach ( $this->required_config as $config ) {
			if ( empty( $this->$config ) ) {
				die( sprintf( __( "The plugin configuration option '%s' must be supplied.", 'add-admin-javascript' ), $config ) );
			}
		}
	...
	protected function load_textdomain() {
		load_plugin_textdomain( 'add-admin-javascript' );
	}
	...
	public function contextual_help( $contextual_help, $screen_id, $screen = null ) {
		...
		$help = '<h3>' . __( 'More Plugin Help', 'add-admin-javascript' ) . '</h3>';
		$help .= '<p class="more-help">';
		$help .= '<a title="' . esc_attr( sprintf( __( 'More information about %1$s %2$s', 'add-admin-javascript' ), $this->name, $this->version ) ) .
			'" class="thickbox" href="' . $help_url . '">' . __( 'Click for more help on this plugin', 'add-admin-javascript' ) . '</a>' .
...
HTML;
	...
	public function help_tabs_content( $screen ) {
		$screen->add_help_tab( array(
			...
			'title'   => __( 'More Help', 'add-admin-javascript' ),
	...
	public function plugin_action_links( $action_links ) {
		$settings_link = '<a href="' . $this->settings_page . '.php?page='.$this->plugin_basename.'">' . __( 'Settings', 'add-admin-javascript' ) . '</a>';
	...
	public function donate_link( $links, $file ) {
		if ( $file == $this->plugin_basename ) {
			...
			$donation_url .= urlencode( sprintf( __( 'Donation for coffee2code plugin: %s', 'add-admin-javascript' ), $this->name ) );
			$title         = __( 'Coffee fuels my coding.', 'add-admin-javascript' );
...
//				echo '<div class="hr">&nbsp;</div>' . "\n";
	...
	public function options_page() {
		...
		echo "<div class='wrap'>\n";
		echo "<div class='icon32' style='width:44px;'><img src='{$logo}' alt='" . esc_attr__( 'A plugin by coffee2code', 'add-admin-javascript' ) . "' /><br /></div>\n";
		...
		echo '<input type="submit" name="Submit" class="button-primary" value="' . esc_attr__( 'Save Changes', 'add-admin-javascript' ) . '" />' . "\n";
		echo '<input type="submit" name="Reset" class="button" value="' . esc_attr__( 'Reset Settings', 'add-admin-javascript' ) . '" />' . "\n";
		echo '</form>' . "\n";
